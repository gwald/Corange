...
//gwald
static void render_shadows_animated_char(renderer* dr, int i, animated_char* ao) {
...
//gwald
static void render_animated_char(renderer* dr, animated_char* ao) {
  ...
  for(int i=0; i < r->num_surfaces; i++) {
    ...
    //gwald if check
    if(material_entry_item(me, "bump_map").as_int > 0)
    shader_program_set_texture(shader, "bump_map", 1, material_entry_item(me, "bump_map").as_asset);

    //gwald if check
    if(material_entry_item(me, "spec_map").as_int > 0)
    shader_program_set_texture(shader, "spec_map", 2, material_entry_item(me, "spec_map").as_asset);

    //gwald if check
    if(material_entry_item(me, "glossiness").as_int > 0)
    shader_program_set_float(shader, "glossiness", material_entry_item(me, "glossiness").as_float);

    //gwald if check
    if(material_entry_item(me, "bumpiness").as_int > 0)
    shader_program_set_float(shader, "bumpiness", material_entry_item(me, "bumpiness").as_float);

    //gwald if check
    if(material_entry_item(me, "specular_level").as_int > 0)
    shader_program_set_float(shader, "specular_level", material_entry_item(me, "specular_level").as_float);

    //gwald if check
    if( material_entry_item(me, "material").as_int > 0)
    shader_program_set_int(shader, "material", material_entry_item(me, "material").as_int);
...
//gwald
    if (dr->render_objects[j].type == RO_TYPE_ANIMATED_CHAR)   { render_animated_char(dr, dr->render_objects[j].animated_char); continue; }
